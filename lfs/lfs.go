...
func (lfs *LFS) CreateFile(
	...
	log.Printf("create file: parent=%v, name=%v, mode=%v, flags=%v",
		op.Parent, op.Name, op.Mode, op.Flags.String())
...
func (lfs *LFS) ReadFile(
	...
	log.Printf("readfile success: id=%v, len=%v, offset=%v, bytesread=%v, dst=%X",
		op.Inode, len(op.Dst), op.Offset, op.BytesRead, op.Dst[:op.BytesRead])
	return nil
}
